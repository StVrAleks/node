...
//****Добавление ссылки "Удалить" в таблицу   
 ...
 fetch('/leftpartREAD',{
   ...
   /* .then(function(response){ return response.json();})
    ...
    .then(function(text) {
       document.getElementById('leftPart').innerHTML = text;})
    .then(function(){
...
let res = await  fetch('/leftpartWRITE',{
    ...
    .then(response = await fetch('/leftpartREADJSON', {method: 'GET'}))
...
let num = document.getElementById('idDelCen').innerHTML;
    ...
    let curBut = document.querySelector('#leftPart #idItem'+num);
    curBut.remove();
    ...
    .then(function(json) {
       ...
       //     butLeft[0].remove();
        for(var i = 0; i < itemsForm.length; i++)
            ...
            clearForm();
            document.getElementById('controlConfig').style.display = 'none';
            })
}
