...
let nameHeader = request.headers["accept"];
...
else if(nameHeader === 'text/html')
    {
        let allStroka = '<ul>Статистика';
        response.setHeader('Content-Type', 'text/html');
        response.setHeader('Content-disposition', 'attachment; filename="stat.html"');
        let filestream = fs.readFileSync("voice.json");
        let clientData = JSON.parse(filestream); 
        for(var i in clientData)
            {
            allStroka = allStroka + `
                        <li>${i} - ${clientData[i]}</li>`;
            }
            allStroka = allStroka + `
                        </ul>`;
...
app.listen(8180, ()=>console.log("Сервер запущен по адресу http://localhost:8180"));